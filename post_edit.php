...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
            $upload_error = '';
            $uploaded = upload_image($_FILES['thumbnail'], 'blog', $upload_error);
            if ($uploaded) {
                if ($post['thumbnail']) {
        ...
        if (empty($errors)) {
            ...
            try {
                $stmt = $pdo->prepare("
                    UPDATE posts 
                    SET title = ?, content = ?, excerpt = ?, category_id = ?, thumbnail = ?, status = ?
                    WHERE id = ?
                ");
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="excerpt" name="excerpt" rows="3"><?php echo clean($post['excerpt']); ?></textarea>
                    </div>
                </div>
                    ...
                    <div class="card bg-light border-0">
                        <div class="card-body">
                            ...
                            <?php if ($post['thumbnail']): ?>
                                <img src="<?php echo UPLOAD_URL . clean($post['thumbnail']); ?>" class="img-fluid rounded mb-2" alt="Current thumbnail">
                            <?php endif; ?>
